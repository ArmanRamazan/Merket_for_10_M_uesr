...
|----------|----------|-------------------|--------|
| **Главная** (каталог + рекомендации) | SSR + streaming | LCP < 1.5s, CLS < 0.1 | ✅ MVP |
| **Поиск** + фильтры | SSR (query) + client (фильтры) | Результат < 200ms, FID < 100ms | ✅ MVP |
| **Карточка курса** | SSR + ISR (revalidate 60s) | LCP < 2s, видео autoplay < 1s | ✅ MVP |
| **Категории** | SSR + ISR | LCP < 2s | 🔴 |
| **Видео-урок** (player) | Client-side (auth protected) | Video start < 2s | 🔴 |
| **Прогресс обучения** | Client-side | INP < 200ms | 🔴 |
| **Регистрация / Логин** | Client-side | TTI < 1.5s | ✅ MVP |
| **Создание курса** | Client-side (auth protected) | FCP < 1s | ✅ MVP |
| **Профиль / Мои курсы** | Client-side (auth protected) | FCP < 1s | 🔴 |
...
### Phase 0: MVP
- Student: каталог курсов, поиск, карточка курса, регистрация с ролями, создание курса
- Teacher: (через student app — создание курса для verified teachers)
- UI Kit: базовые компоненты, CourseCard
- Нет: admin panel, video player, analytics, i18n
