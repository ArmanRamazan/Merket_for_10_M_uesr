...
class LessonService:
    ...
    async def _check_owner_via_module(self, module_id: UUID, teacher_id: UUID) -> None:
    ...
    async def create(
        ...
        await self._check_owner_via_module(module_id, teacher_id)
        return await self._repo.create(module_id, title, content, video_url, duration_minutes, order)
    ...
    async def update(
        ...
        if not lesson:
            raise NotFoundError("Lesson not found")
        await self._check_owner_via_module(lesson.module_id, teacher_id)
