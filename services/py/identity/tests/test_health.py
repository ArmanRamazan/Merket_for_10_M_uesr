...
def test_app(mock_pool):
    app = FastAPI()
    app.include_router(create_health_router(lambda: mock_pool))
    return app
...
async def test_readiness_db_down():
    ...
    app = FastAPI()
    app.include_router(create_health_router(lambda: pool))
    async with AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as c:
        resp = await c.get("/health/ready")
    assert resp.status_code == 503
    assert resp.json()["checks"]["postgres"] == "unavailable"
