...
from app.domain.user import User, TokenPair
...
class AuthService:
    ...
    async def register(self, email: str, password: str, name: str) -> TokenPair:
        ...
        user = await self._repo.create(email, password_hash, name)
        token = create_access_token(
            str(user.id), self._jwt_secret, self._jwt_algorithm, self._jwt_ttl_seconds
        )
